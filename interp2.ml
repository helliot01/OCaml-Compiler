...
type command =
  ...
  | Call
  | Throw
  | TC of (command list * command list)
...
and deffun () = 
  ((keyword "DefFun") >> (name << ws) +++ (name << ws) +++ many' command) << 
  (keyword "End") >|= (fun ((fname, arg), cmds) -> DefFun (fname, arg, cmds))

and tc () = 
  ((keyword "Try") >> many' command) +++ 
  ((keyword "Catch") >> many' command) << (keyword "End") >|= 
  (fun (cmds1, cmds2) -> TC (cmds1, cmds2))
...
type value =
  ...
  | N_val of string
  | U_val
  | F_val of (env*string*string*command list)
...
let rec run (p : prog) (st : stack) (e : env) (log : string list) :
  ...
  | Ifte (cmds1, cmds2) :: rest ->
    (match st with
     ...
     | _ :: st -> log, TypeError
     | _ -> log, StackError)
  | DefFun (fname, arg, cmds) :: rest -> run rest st (put e fname (F_val (e,fname,arg,cmds))) log
  | Call :: rest -> 
    (match st with
    | x :: F_val y :: st -> 
        (match y with g,fname,arg,cmds -> 
          match (run cmds [] (put (put g arg x) fname (F_val y)) log) with 
          |log, Ok (v:: _) -> run rest (v::st) e log
          |log, Ok _ -> log, StackError
          |error -> error
          )
    | x :: y :: st -> log, TypeError
    | _ -> log, StackError
        )
  ...
  | TC (cmds1, cmds2) :: rest ->
    ...
    |(log, Ok (v :: _ )) -> run rest (v::st) e log 
    |(log, Ok _ ) -> log, StackError
    |(log, result) -> (match run cmds2 ((I_val (to_int_result result))::st) e log with 
                  |(log, Ok (v)) -> run rest (v@st) e log
